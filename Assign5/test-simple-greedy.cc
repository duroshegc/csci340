...
int main(int argc, char *argv[]) {
  std::cout << "********************************************************************************\n";
  std::cout << "* test-simple-greedy                                                            \n";
  std::cout << "*  This test program tests your split_string_greedy function in isolation. A    \n";
  ...
  std::cout << "********************************************************************************\n";
  std::cout << "\n";
  const std::string default_string = "  This is    a  long   string  with   lots   of spaces    and you need to greedily  split it.  ";
  const char default_delim = ' ';
  std::string input_string;

  // if no command line arguments, use default string and delim
  if(argc < 2) {
    test_split_string_greedy(default_string, default_delim);
  }
  else if(argc < 3) {  // 1 argument, new string comes from 1st command line arg
    test_split_string_greedy(argv[1], default_delim);
  }
  else { // 2 arguments, new string comes from 1, delimiter comes from 2
    test_split_string_greedy(argv[1], argv[2][0]);
  }
